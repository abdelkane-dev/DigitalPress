...
def is_sandbox_mode() -> bool:
    ...
    return MOVAPAY_API_KEY in _PLACEHOLDER_VALUES
...
def verify_webhook_signature(raw_body: bytes, signature_header: str) -> bool:
    ...
    if not signature_header or not MOVAPAY_SECRET:
        return False

    expected = hmac.new(
        MOVAPAY_SECRET.encode('utf-8'), raw_body, hashlib.sha256
    ).hexdigest()
...
class MovapayService:
    ...
    def __init__(self):
        self.base_url = MOVAPAY_BASE_URL
        self.api_key = MOVAPAY_API_KEY
        self.secret = MOVAPAY_SECRET
